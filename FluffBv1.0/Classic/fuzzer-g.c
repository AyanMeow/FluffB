...
void tryFuzzing(char devAddr[ADDR_MAX_LEN])
{
    ...
    printf("[-] please choose one target protol(1~5):\n");
    printf("[+] 1.L2CAP\n");
    printf("[+] 2.RFCOMM\n");
    printf("[+] 3.OBEX\n");
    printf("[+] 4.SDP\n");
...
// {
//     int maxsize=AVCTP_MAXSIZE;
//     char *sbuf,*rbuf,*buf;
//     int NoPackets,NoCrush;//number of packets & crushes
//     int sock,NoD;
//     int divided=0;//是否需要分片
//     int isStart;
//     int size;
//     NoCrush=0;
//     NoPackets=0;
...
//     if(!(buf = (char *) malloc ((int) maxsize + 1))) {
// 		perror("malloc");
// 		exit(EXIT_FAILURE);
// 	}//分配保存包内存
...
//                 divided=1;
//                 isStart=1;
//                 NoD=ceil((1.0*size)/maxsize);
//             }//随机包大小超过限制，设置分片标记
//         }
//         if(size<=4) size=4;
//         if(!(sbuf = (char *) malloc ((int) maxsize + 1))) {
// 		    perror("malloc");
// 		    exit(EXIT_FAILURE);
// 	    }//分配发送包内存

//         if(reciveRsp&&!(rbuf = (char *) malloc ((int) maxsize + 1))) {
//             perror("malloc");
// 		    exit(EXIT_FAILURE);
//         }//分配接受包内存

//         bzero(sbuf, (size<=maxsize)?size:maxsize);
//         size-=maxsize;//指示是否有后继包
// 		for(int i=0 ; i<size ; i++)	
// 			sbuf[i] = rand();
//         if(!divided)
//         {
//             AVCTP_SIN* as=(AVCTP_SIN*)sbuf;
//             as->byte0.IPID=0;
//             as->byte0.CoR=0;
//             as->byte0.PacketType=AVCTP_PT_SINGLE;
//             as->byte0.TransactionLabel=rand()%0x10;
//             as->PID=AVCTP_PID;
//         }//非分片包
//         if(divided)
//         {
//             if(isStart)
//             {
//                 AVCTP_START* as=(AVCTP_START*)sbuf;
//                 as->byte0.IPID=0;
//                 as->byte0.CoR=0;
//                 as->byte0.PacketType=AVCTP_PT_START;
//                 as->byte0.TransactionLabel=rand()%0x10;
//                 as->PID=AVCTP_PID;
//                 isStart=0;
//             }//分片起始包
//             else if(size>0)
//             {
//                 AVCTP_CONTINUE* as=(AVCTP_CONTINUE*)sbuf;
//                 as->byte0.IPID=0;
//                 as->byte0.CoR=0;
//                 as->byte0.PacketType=AVCTP_PT_CONTINUE;
//                 as->byte0.TransactionLabel=rand()%0x10;
//             }//分片中间包
//             else
//             {
//                 AVCTP_END* as=(AVCTP_END*)sbuf;
//                 as->byte0.IPID=0;
//                 as->byte0.CoR=0;
//                 as->byte0.PacketType=AVCTP_PT_END;
//                 as->byte0.TransactionLabel=rand()%0x10;
//                 divided=0;
//             }//分片结束包
//                         size=RandomVariation(sbuf,size);
//             printf("%d,%d\n",++NoPackets,size);
//             if(send(sock,sbuf,size,0)<=0){
//                 printf("==================================================\n");
//                 printf("possible crush! Host:%s size:%d\n",tarAddr,size);
//                 printf("==================================================\n");
//             }
//             if(reciveRsp)
//             {
//                 recv(sock,rbuf,size,0);
//                 fuzzlogSR(sbuf,rbuf,4);
//             }
//             else
//             {
//                 fuzzlog(sbuf,4);
